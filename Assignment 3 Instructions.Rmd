...
K1 <- data.frame(Class_Motivation)
...
#Hypothosis: If we recode the values, this may support in keeping all of the completed data available, but it will slightly skew any calculations we do upon the data, for example, if we turn N/A into the mean, then the clusters will change slighlty as well.

#If we delete the rows with the N/A, then we lose some data and that will skew the mean and clusters as well. In a data set this small, this could be significant.
...
ggplot(K6, aes(week, avg, colour = cluster)) + geom_line() + ggtitle("STUDENT MOTIVATION RESULTS OVER 5 WEEKS") + xlab("Week") + ylab("Average Motivation")

```

What patterns do you see in the plot?
#students in cluster 1 start out with little motivation, and at week 3, their motivation increases drastically. In cluster 2, the opposite happens. They start out highly motivated and at week 3, they begin to lose motivation drastically.
...
#First Part of Part II - create a cluster of the answers to the questions.
#1. convert data to data frame (K8)
#2. omit names and select the rest of the columns, 3 to 12
#3. convert column Cat and Gif to 1s and 2s: ifelse(df$column=="word",1,2)
#3. standarize the range w/scale.

```{r}
K8 <- data.frame(HUDK4050_2017_Cluster_Survey)
K9 <- dplyr::select(K8, 3:12)
K9$cat <- ifelse(K9$cat=="Yes",1,2)
K9$gif.jiff <- ifelse(K9$gif.jiff=="g-iff",1,2)
Komit <- na.omit(K9)
K9 <- scale(K9) 

#4. create cluster - let's choose 2 clusters (for now :))
#5. Attach these clusters to K9
fit2<-kmeans(K9, 5)
K10 <- data.frame(K9, fit2$cluster)
```

```{r}

library(cluster)
clusplot(K9, fit2$cluster, color=TRUE, labels=1, lines=0, main="Dr. Lang's Favorite Class Data Results")

```
```{r}
#Second Part of Part II - Regions where people grew up
#1. create K.regions data frame to isolate the regions
#2. select the column for the city (for now, add state once it works :))
#3. use ggmap & maps

K.regions <- dplyr::select(K8, 15)
library("ggmap", lib.loc="C:/Program Files/R/R-3.4.1/library")
library("maps", lib.loc="C:/Program Files/R/R-3.4.1/library")

#change United States to USA (this took FOREVER to figure out! Tried replace, rename)

Countries <- ifelse(K.regions$country.grew=="United States","USA",
                    ifelse(K.regions$country.grew=="China","China",
                    ifelse(K.regions$country.grew=="Indonesia","Indonesia", 
                    ifelse(K.regions$country.grew=="Taiwan","Taiwan","USA"))))

DFC <- data.frame(Countries)

# get countries latitude/longitude from Google Maps API
LL <- geocode(as.character(DFC$Countries), output = "latlon")

#Join the lat and lon to DFC
DFC <- cbind(DFC,LL)
...
DFC$Countries <- ifelse(DFC$Countries=="China",1,
                 ifelse(DFC$Countries=="USA",2,
                 ifelse(DFC$Countries=="Taiwan",3,4)))

DFC <- DFC %>% group_by(lon, lat)
fit3 <- kmeans(DFC, 4)
KC <- data.frame(DFC, fit3$cluster)

ggplot(DFC, aes(Countries, color = fit3$cluster)) + geom_jitter(width=2, height = 3) + ggtitle("Where Students Grew Up") + xlab("lat") + ylab("lon")
```
